...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const token = searchParams.get("invite") ?? hashParams.get("invite") ?? undefined;
    ...
    window.history.replaceState({}, document.title, window.location.pathname + (cleanQuery ? `?${cleanQuery}` : "") + (cleanHash ? `#${cleanHash}` : ""));
    setInvitationLoading(true);
    fetch("/api/invitations/resolve", {
      method: "POST",
...
function ApprovalsView({ setNotice }: { setNotice: (value: string) => void }) {
  ...
  const update = async (userId: string, action: "approve" | "suspend") => {
    ...
    try {
      ...
      setNotice(data.warning ? `Advertencia: ${data.warning}` : "Cuenta actualizada correctamente.");
      void load();
...
function Rooms({ setNotice, guestMode = false, lockedRoom }: { setNotice: (value: string) => void; guestMode?: boolean; lockedRoom?: string }) {
  ...
  const loadGuestAccess = async () => {
    ...
    setGuestAccess(await response.json());
  };

  const inviteGuests = async () => {
    if (!isCreator || actionLoading) {
      if (!isCreator) setNotice("Solo el creador de la llamada puede invitar participantes.");
      return;
    }
    setActionLoading("load-guests");
    setInviteOpen(true);
    try {
      await loadGuestAccess();
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "No se pudo cargar la lista de invitados.");
    } finally {
      setActionLoading(null);
    }
  };
  ...
  const revokeGuest = async (userId: string) => {
    ...
    setNotice(result.liveKitRevoked === false
      ? "Advertencia: el acceso se revocó, pero no se pudo expulsar al invitado conectado."
      : "Acceso del invitado revocado.");
    await loadGuestAccess();
    } catch {
  ...
  const cancelMeeting = async (meetingId: string) => {
    ...
    setNotice(data.liveKitRevoked === false
      ? `Advertencia: reunión cancelada, pero no se pudo expulsar a todos los participantes de LiveKit.`
      : "Reunión cancelada.");
    } catch {
        ...
        <div className="meeting-shell">
          <div className="meeting-stage" ref={meetingStageRef}>
            ...
            <div className="meeting-controls">
              ...
              {!guestMode && isCreator && <button className="control-button" onClick={inviteGuests} type="button" disabled={actionLoading === "load-guests"}>{actionLoading === "load-guests" ? "Cargando..." : "Invitar"}</button>}
